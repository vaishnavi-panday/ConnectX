...
const ChatPage = () => {
  ...
  const fetchMessages = async () => {
    try {
      const res = await api.get(`/message/${id}/chat`);
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      await api.post(`/message/${id}/chat`, formData);
  ...
  const handleReaction = async (messageId, emoji) => {
    try {
      const res = await api.patch(`/message/${messageId}/react`, {
        reaction: emoji,
      });
  ...
  useEffect(() => {
    const loadChat = async () => {
      ...
      await api.patch(`/message/seen/${id}`);
      ...
      <div className="max-w-4xl mx-auto mt-6 bg-white/70 backdrop-blur-xl rounded-3xl shadow-2xl overflow-hidden border border-[#FFE5D8]">
        <div className="sticky top-0 z-20 bg-white/80 backdrop-blur-md border-b border-[#FFE5D8] p-4 flex items-center gap-4">
          ...
          <div className="flex-1">
            <h2 className="font-bold text-gray-800">{receiver?.username}</h2>

            {isTyping && (
              <p className="text-xs text-[#FF7F66] animate-pulse">typing...</p>
            )}
          </div>
        ...
        {replyMessage && (
          <div className="bg-[#FFF5EF] border-l-4 border-[#FF7F66] px-4 py-2 flex justify-between items-center">
            <div>
              ...
              <p className="text-sm text-gray-700">{replyMessage.text}</p>
            </div>
